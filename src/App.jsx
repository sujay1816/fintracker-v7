    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">
      ...
      <div className="relative flex">
        ...
        <aside className={`${mobile ? 'fixed top-14 left-0 h-[calc(100vh-56px)] z-50' : 'sticky top-0 h-screen'} ${sidebar ? 'w-56 translate-x-0' : mobile ? '-translate-x-full w-56' : 'w-16'} bg-slate-900/95 backdrop-blur-xl border-r border-white/10 p-3 transition-all duration-300 flex flex-col overflow-y-auto`}>
          ...
          {/* Save status */}
          {sidebar && (
            <div className={`hidden md:flex items-center gap-2 px-2 py-1.5 rounded-lg mb-4 text-xs ${save === 'saving' ? 'bg-yellow-500/10 text-yellow-400' : save === 'error' ? 'bg-red-500/10 text-red-400' : 'bg-green-500/10 text-green-400'}`}>
              {save === 'saving' ? <Loader2 size={12} className="animate-spin" /> : <Cloud size={12} />}
